...
<<<<<<< HEAD
  ...
  return [...addresses]
    .sort()
    .map(address => ({ address, url: `http://${address}:${port}` }));
}

async function inspectPathWithTimeout(value) {
  let timer;
  try {
    return await Promise.race([
      ...
      new Promise(resolve => {
        timer = setTimeout(() => resolve({
          ...
          writable: null,
          empty: null,
          error: 'Path check timed out. Verify the network connection and permissions.'
        }), 12_000);
      })
    ]);
  } finally {
...
<<<<<<< HEAD
    ...
    "  $folder = $shell.BrowseForFolder([long]$owner.Handle, 'Select a local or network folder for Task Manager', 0x51, 0)",
    '  if ($folder) {',
...
app.get('/api/state', (req, res) => {
  ...
  res.json({
    ...
    tasks: snapshot.tasks,
    runs: snapshot.runs.slice(0, 150),
    cleanups: snapshot.cleanups.slice(-500).reverse()
  });
});
...
<<<<<<< HEAD
  ...
  return res.json({ mode: 'native', path: selectedPath, cancelled: !selectedPath });
}));
